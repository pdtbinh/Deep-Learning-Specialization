...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = (np.sum(e*g)/np.sum(np.square(g))) * g
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = np.sum(mu * bias_axis)/np.sum(np.square(bias_axis)) * bias_axis
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = np.sum(e_w1 * bias_axis)/np.sum(np.square(bias_axis)) * bias_axis
    e_w2B = np.sum(e_w2 * bias_axis)/np.sum(np.square(bias_axis)) * bias_axis
